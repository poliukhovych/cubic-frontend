...
export default function RoleSelectionPage() {
  ...
  // Get pending Google token from session storage
  const [idToken, setIdToken] = useState<string | null>(null);

  useEffect(() => {
    const token = sessionStorage.getItem('pending_google_token');
    if (!token) {
      // If no pending token, redirect to login
      navigate('/login');
    } else {
      setIdToken(token);
    }
  ...
  const handleRoleSelect = async () => {
    ...
    try {
      const authResponse = await authenticateWithGoogle(idToken, selectedRole);
      ...
      } else if (authResponse.user.role === UserRole.TEACHER) {
      ...
      setError(error instanceof Error ? error.message : 'Помилка аутентифікації');
    } finally {
      setIsLoading(false);
    }
  };
