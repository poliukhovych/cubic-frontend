...
import type { GradeItem, StudentGradesResponse, SubjectGrades } from "@/types/grades";
...
/* ──────────────────────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────────────────── */
const SUBJECT_META: Record<CanonSlug, { name: Canon; teacher: { id: string; name: string; email?: string } }> = {
  [slugify("Математика")]:              { name: "Математика",              teacher: { id: "t_math", name: "проф. Ірина Коваль", email: "i.koval@univ.edu" } },
  [slugify("ОПП")]:                     { name: "ОПП",                     teacher: { id: "t_oop", name: "доц. Андрій Петренко" } },
  [slugify("Алгоритми і структури даних")]: { name: "Алгоритми і структури даних", teacher: { id: "t_algo", name: "д-р Сергій Литвин" } },
  [slugify("Англійська мова")]:         { name: "Англійська мова",         teacher: { id: "t_eng", name: "Ольга Руденко" } },
  [slugify("Бази даних")]:              { name: "Бази даних",              teacher: { id: "t_db", name: "ас. Марія Гнатюк" } },
  [slugify("Комп'ютерні мережі")]:      { name: "Комп'ютерні мережі",      teacher: { id: "t_net", name: "доц. Дмитро Сай" } },
  [slugify("Фізика")]:                  { name: "Фізика",                  teacher: { id: "t_phy", name: "проф. Олег Вербицький" } },
  [slugify("Теорія ймовірностей")]:     { name: "Теорія ймовірностей",     teacher: { id: "t_prob", name: "доц. Наталія Таран" } },
  [slugify("Операційні системи")]:      { name: "Операційні системи",      teacher: { id: "t_os", name: "ас. Володимир Дяченко" } },
  [slugify("Комп'ютерна графіка")]:     { name: "Комп'ютерна графіка",     teacher: { id: "t_cg", name: "ас. Аліса Кравець" } },
  [slugify("Філософія")]:               { name: "Філософія",               teacher: { id: "t_phil", name: "проф. Лариса Бойко" } },
  [slugify("Веб-технології")]:          { name: "Веб-технології",          teacher: { id: "t_web", name: "доц. Павло Козак" } },
  [slugify("Проєктний практикум")]:     { name: "Проєктний практикум",     teacher: { id: "t_proj", name: "ментор Ірина Бондар" } },
  [slugify("Англійська мова (розмовна)")]: { name: "Англійська мова (розмовна)", teacher: { id: "t_eng2", name: "Ольга Руденко" } },
  [slugify("ОПП (практика)")]:          { name: "ОПП (практика)",          teacher: { id: "t_oop_lab", name: "доц. Андрій Петренко" } },
  [slugify("Бази даних (лаб.)")]:       { name: "Бази даних (лаб.)",       teacher: { id: "t_db_lab", name: "ас. Марія Гнатюк" } },
};
...
export async function fetchStudentSchedule(studentId: string): Promise<StudentSchedule> {
  ...
  return ok({
    ...
    lessons: [
      ...
      L(5,2,"Алгоритми і структури даних","ауд. 221","any", meet("alg-333-222")),
...
export async function fetchStudentHomework(studentId: string): Promise<StudentHomeworkResponse> {
  ...
  const mkSubjectHW = (canonId: string, displayName: string, teacherId: string) => ([
    T(displayName, `${displayName}: Домашка №1`, -18, teacherId, `https://classroom.google.com/c/${canonId}/a/HW1`),
    T(displayName, `${displayName}: Домашка №2`, -4,  teacherId, `https://classroom.google.com/c/${canonId}/a/HW2`),
    T(displayName, `${displayName}: Домашка №3`, 9,   teacherId, `https://classroom.google.com/c/${canonId}/a/HW3`),
  ]);

  // згенеруємо для всіх предметів з SUBJECT_META
  const tasks: HomeworkTask[] = Object.entries(SUBJECT_META).flatMap(([canonId, meta]) => {
    const hw = mkSubjectHW(canonId, meta.name, meta.teacher.id);
    return hw;
  });

  // бонус: одна додаткова “аліасна” ДЗ для БД як "БД"
  const bdId = Object.keys(SUBJECT_META).find(k => SUBJECT_META[k].name === "Бази даних");
  if (bdId) {
    tasks.push(
      T("БД", "БД: Додаткове завдання (аліас-перевірка)", 3, SUBJECT_META[bdId].teacher.id, `https://classroom.google.com/c/${bdId}/a/ALIAS`)
    );
  }
...
export async function fetchStudentGrades(studentId: string): Promise<StudentGradesResponse> {
  ...
  const mkSubjectGrades = (canonId: string, displayName: string) => ([
    mk(displayName, 8,  `${displayName}: Контрольна/квіз`,       21, `https://classroom.google.com/c/${canonId}/a/G1`, 10),
    mk(displayName, 9,  `${displayName}: Лабораторна/практика`,   8, `https://classroom.google.com/c/${canonId}/a/G2`, 10),
    mk(displayName, 10, `${displayName}: Микрокейс/есе`,          2, `https://classroom.google.com/c/${canonId}/a/G3`, 10),
  ]);
  ...
  const bdCanon = Object.keys(SUBJECT_META).find(k => SUBJECT_META[k].name === "Бази даних");
  if (bdCanon) {
    allItems.push(mk("БД", 10, "БД: Захист лаби (аліас)", 5, `https://classroom.google.com/c/${bdCanon}/a/G-ALIAS`, 10));
  }
  ...
  const subjects: SubjectGrades[] = Array.from(buckets.entries()).map(([canonId, b]) => {
...
/* ──────────────────────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────────────────── */
export async function fetchStudentSubject(studentId: string, subjectId: string): Promise<SubjectDetails> {
  ...
  const meta = SUBJECT_META[canonId] ?? { name: (subjectId as any), teacher: { id: "t", name: "Викладач" } };

  // ✅ перше посилання на пару (з урахуванням аліасів), і завжди string
  const meetingUrl = schedule.lessons.find(l => matchById(canonId, l.subject))?.meetingUrl ?? "";

  const materials: SubjectMaterial[] = [
    { id: uid(), title: "Програма курсу",         url: "https://drive.google.com/file/d/course",  kind: "doc" },
    { id: uid(), title: "Слайди лекції 1",        url: "https://drive.google.com/file/d/slides1", kind: "slides" },
    { id: uid(), title: "Відео лабораторної №1",  url: "https://youtu.be/dQw4w9WgXcQ",            kind: "video" },
    { id: uid(), title: "Приклад репозиторію",    url: "https://github.com/example/course",       kind: "repo" },
  ];
  ...
  const upcomingHomework = hw.tasks
    ...
    .slice(0, 5)
    .map(t => ({ id: t.id, text: t.text, dueDate: t.dueDate, classroomUrl: t.classroomUrl }));
  ...
  return ok({
    ...
    meetingUrl,
    description: "Короткий опис курсу, критерії оцінювання, політика дедлайнів, посилання на Classroom.",
    materials,
    upcomingHomework,
    recentGrades,
